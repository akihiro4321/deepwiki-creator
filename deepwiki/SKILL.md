---
name: deepwiki
description: コードベースを解析し、DeepWiki のような包括的なWikiドキュメントを自動生成、または特定の技術的な質問に高品質な回答を提供するスキル。ユーザーが「Wikiを作成して」「コードベースをドキュメント化して」と依頼した場合（Wiki一括生成モード）、または「〇〇の実装はどうなっているの？」「〇〇機能のアーキテクチャを解説して」等と特定の質問をした場合（Ad-hoc Q&Aモード）に使用する。アーキテクチャ図（Mermaid）やコードスニペットを活用した詳細なドキュメント・回答を生成する。
---

# DeepWiki - コードベース分析＆ドキュメント生成スキル

リポジトリを深く解析し、包括的なWikiドキュメント（DeepWiki）の自動生成、および高度な技術的質問への回答（Q&A）を提供する。
...
## モードの選択: Wiki一括生成 vs Ad-hoc Q&A

ユーザーの依頼内容に応じて、以下の2つのモードを使い分ける。

### モードA: Wiki一括生成（デフォルト）

「Wikiを作成して」「全体像をまとめて」等、リポジトリ全体のドキュメント化を求められた場合に使用する。本ドキュメントの **Phase 1 〜 Phase 5** の手順をすべて実行する。

### モードB: Ad-hoc Q&A (詳細調査 & 回答)

「〇〇の処理フローを教えて」「このモジュールの関連クラスをDeepWiki基準で教えて」等、特定の機能やモジュールについてのみ質問された場合に使用する。Wikiファイル群は出力せず、チャットの回答として直接ユーザーに高品質なドキュメントを提供する。

1. **対象モジュールの特定と検索**: 質問に関連するエントリーポイントや主要ファイルを「ファイル検索ツール」や「ディレクトリ一覧抽出ツール」で特定する。
2. **Deep Research (詳細調査)**: 関連ファイルを「ファイル閲覧ツール」で読み、さらに呼び出し元や依存先を深堀りする。
3. **回答生成**: Phase 3b の品質基準（Sources行、実名Mermaid図、コードスニペット）を満たしたMarkdownを生成し、チャットで回答する。

---

## Phase 1: 構造分析 (Wiki一括生成時)
